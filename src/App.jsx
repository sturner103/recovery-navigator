            ...
            <div className="crisis-resources">
              {Object.entries(crisisResources).map(([key, region]) => (
                <div key={key} className="crisis-region"><h3>{region.name}</h3>
                  {region.resources.map((r, idx) => <div key={idx} className="crisis-resource"><span className="resource-name">{r.name}</span>{r.phone && <a href={`tel:${r.phone.replace(/\s/g, '')}`} className="resource-phone">{r.phone}</a>}{r.url && <a href={r.url} target="_blank" rel="noopener noreferrer" className="resource-link">Find support →</a>}<span className="resource-desc">{r.description}</span></div>)}
                </div>
              ))}
            </div>
            <div className="crisis-footer"><button onClick={exitAssessment} className="secondary-button">Return home</button></div>
          </div>
        </main>
      </div>
    );
